...
Direction_searchtuning <- function(X, loading, weight, deriv, resol=1.5, maxiter=10){
  ...
  iter = 1
  v = Variable(p+1)
  obj = 1/4*sum((CVXR::power(X%*%H%*%v, 2))*weight*deriv)/n + sum((loading/loading.norm)*(H%*%v)) + mu*sum(abs(v))
  # adj.XH = sqrt(weight)*sqrt(deriv)*(X%*%H)
  # P = t(adj.XH)%*%adj.XH
  # obj <- 1/4*quad_form(v, P)/n + sum((loading/loading.norm)*(H%*%v)) + mu*sum(abs(v))
